...
ALL sub-skills dispatch via Task tool for context isolation:

| Sub-skill | Dispatch method |
|-----------|----------------|
| `red-*` | `Task tool` (agent: `red-agent`) — pass layer, story folder path, scenario name, and ADR content (if loaded) |
| `green-*` (except `green-acceptance`) | `Task tool` (agent: `green-agent`) — pass layer, story folder path, scenario name, and ADR content (if loaded) |
...
| `/refactor` | `Task tool` (agent: `refactor-agent`) |
| `/test-review` | `Task tool` (agent: `test-review-agent`) |
| `/test-coverage` | `Task tool` (agent: `coverage-agent`) |
...
**CHAINING: After each sub-step completes (Task tool return), echo a 1-2 line status summary (agent name, outcome, pass/fail counts) to the user, then immediately dispatch the next sub-step. Do NOT wait for user input between sub-steps — the echo is informational only.**
...
**LOG REMINDER: Every time you dispatch a sub-agent (Task tool call), output this line immediately before the call:**
```
