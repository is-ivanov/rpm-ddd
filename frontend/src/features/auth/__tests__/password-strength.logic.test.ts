...
describe('Activation Password Strength Classification', () => {
  ...
  it('classifies a short lowercase-only password as weak (E2E-pinned input)', async () => {
    await issue('189');

    expect(computePasswordStrength('weak')).toBe('weak');
  });

  it('classifies a value satisfying all six complexity rules as strong (E2E-pinned input)', async () => {
    await issue('189');

    expect(computePasswordStrength('Str0ng-P@ssw0rd!')).toBe('strong');
  });

  it('treats a value satisfying three or fewer rules as weak', async () => {
    await issue('189');

    expect(computePasswordStrength('abcABC')).toBe('weak');
  });

  it('treats a value satisfying four rules as medium (lower band boundary)', async () => {
    await issue('189');

    expect(computePasswordStrength('abcdef1!')).toBe('medium');
  });

  it('treats a value satisfying five rules as medium (upper band boundary)', async () => {
    await issue('189');

    expect(computePasswordStrength('Abcdefghijk1')).toBe('medium');
  });

  it('classifies an empty value as weak', async () => {
    await issue('189');
