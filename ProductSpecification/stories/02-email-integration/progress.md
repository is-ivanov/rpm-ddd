...
- [~] green-adapter scheduling ⛔ **BLOCKED — ADR amendment pending (decide A vs B), see `SESSION-HANDOFF-2026-06-01.md`.** Modulith's `StalenessMonitorConfiguration` enables scheduling app-wide, so gating `SchedulingConfiguration` does NOT stop the job's `@Scheduled` from firing under the `test` profile → breaks 5.1. Fix: move `@Scheduled` off the always-present job onto a conditionally-created bean (Option B recommended: trigger method on the gated `SchedulingConfiguration`; job becomes ungated logic-only). Green WIP is in a git stash. — implement per ADR `resubmit-scheduling-wiring`: on `SchedulingConfiguration` add `@EnableScheduling` + `@EnableSchedulerLock` + `@ConditionalOnProperty(rpm.scheduler.enabled, matchIfMissing=true)` + `@Bean JdbcTemplateLockProvider`; bind `EventResubmitProperties` (`@ConfigurationProperties`/`@Validated`/`@NotNull`) + register it; add `@Scheduled(fixedDelayString="${rpm.events.resubmit.interval}")`/`@SchedulerLock` to the job; `rpm.events.resubmit.interval: 5s` in `application.yml`, `rpm.scheduler.enabled: false` in `application-test.yml`, `shedlock` Liquibase changeset. ShedLock 7.7.0 compat already verified. Heads-up: once `@EnableScheduling` is live the `fixedDelay` job fires during the wiring test — keep harmless via the mock `IncompleteEventPublications`.)
