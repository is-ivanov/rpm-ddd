...
- [x] demo (recorded Scn 3.3 relative-time + hover-tooltip E2E in headless slowMo=2000 + video on; 1 passed (7.4s); recording → frontend/test-results/demo-users-grid-time-tooltip.webm (gitignored). FE-only (page.route mocks + page.clock.setFixedTime, no backend). playwright.config.ts demo edits reverted, working tree clean.)
...
- [x] red-acceptance (extended CurrentUserInfoIntegrationTest.should_returnOwnUserInfo_when_authenticated: added `"timeZone": "UTC"` to the whole-response expected JSON — one new consequence on the same single `me(adminSession)` action, not a new test. @ExpectedToFail(withExceptions = AssertionError.class) marker. RED confirmed by running WITHOUT marker first: HTTP 200, then jsonunit strict-equality AssertionFailedError `missing: "timeZone"` (CurrentUserResponse has no timeZone field yet — green-acceptance adds it). Prediction all-YES (Type AssertionError / Message missing timeZone / Status 200+body-fail). With marker: 1 run/0 fail/1 skip (ExpectedToFail aborts), BUILD SUCCESS. assertOk = jsonunit json().isEqualTo no-options = strict; opentest4j AssertionFailedError IS-A AssertionError so the AssertionError.class pin matches. test-review CLEAN (strict whole-response, deterministic "UTC" literal, black-box — only AuthApi+AuthSessionFactory injected, no storage port). refactor CLEAN no-op (idiomatic 44-line test). checkstyle+pmd green; IDE get_file_problems errors:[]. Deferred-foundation scenario → no issue tag.)
...
- [x] green-acceptance (added `String timeZone` to the CurrentUserResponse record (after status, before roles) + mapped `user.getTimeZone().getId()` in from(); removed @ExpectedToFail. Simple-delegation plumbing exception. CurrentUserInfoIntegrationTest GREEN 1/0/0. COLLATERAL fixed (required-field wiring, mirrors Scn 3.1's web-slice JSON fix): ActivateAccountMassAssignmentIntegrationTest also asserts the whole /me body via authApi.me(session).assertOk(...) — its expected JSON lacked timeZone so strict jsonunit failed on the extra key; added `"timeZone": "UTC"` (the activated user is registered via ActivationTokenFixture which posts timeZone="UTC"). Full suite 161/0/0 BUILD SUCCESS (1st run caught the 1 collateral failure, 2nd run after fix all green). checkstyle+pmd green; IDE get_file_problems errors:[] on all 3 files. Backend-foundation block COMPLETE — closes the endpoints.md /me timeZone contract gap that Scn 3.3 green-frontend-api exposed (live /api/auth/me now emits timeZone; FE schema parse no longer breaks against the real backend / full-stack journey).)
