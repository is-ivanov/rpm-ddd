...
- [x] red-frontend (2 new pure logic functions for the Created cell: `toRelativeTimeLabel(iso, now)` + `toAbsoluteTooltipParts(iso, tz)` → new `AbsoluteTimeParts{date,time,tzLabel,ianaZone}` type. RED pass-through stubs (returns raw ISO / empty parts). New `users-grid.timestamps.logic.test.ts`: 6 it.fails — 2 relative-label (17 days ago at 17.14d delta, "just now" <60s) + 4 tooltip parts (date 2026-06-12, time 11:14 CEST, tzLabel CEST, ianaZone Europe/Berlin verbatim). All expected values lock-step with users-grid-time.fixture.ts. RED: stubs return wrong values → AssertionError; prediction all-YES (6 field rows). it.fails locked: 73 passed/6 expected-fail/0 skip, build green. test-review CLEAN (0 edits — strict toBe throughout, all 4 AbsoluteTimeParts fields pinned, 7 lock-step values match the E2E fixture). refactor: Extract Method on buildUserRows (12→3-line delegator + `toUserRow` helper, `const { audit } = user` destructure) — pre-existing Scn 2.1 code in the same modified file, A1 structural smell fix; RED stubs untouched, no test changes. lint(oxlint/eslint/prettier/type-check) green; IDE get_file_problems errors:[] on all 3 files; all <200. Story scenario → no issue tag.)
- [~] green-frontend
- [ ] red-frontend-api
- [ ] green-frontend-api
- [ ] align-design
