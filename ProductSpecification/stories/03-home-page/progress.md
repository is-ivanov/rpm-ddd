...
- [x] align-design (Build component: wired the "Выйти" button in `UserMenu.vue` to `handleLogout()` = `await logout()` then `window.location.reload()` (user-chosen: full reload → re-fetch `/me` → 401 → welcome, honoring the DSL). Loading-state (user-chosen: disable-only) via `loggingOut` ref + `:disabled` + `disabled:` utilities — prevents double-logout, no spinner since the page reloads on success. No mockup/markup restyle, so `/align-design` + `/design-review` are no-ops (no placeholder data). `/refactor` clean (component <100 lines; `handleLogout` is a cohesive disable-around-async like `submitLogin`; orchestration only, no business logic). Verified: lint green, 45 unit tests pass, user-menu 3.1 still green, playwright 4.3 now "Expected to fail, but passed" (marker removed in green-playwright). `/test-coverage frontend --focus` not meaningful — only a presentational `.vue` changed (covered by the playwright tier); `logout.api.ts` already unit-covered in green-frontend-api.)
- [x] green-playwright (removed the `test.fail()` marker + stale RED comment — the only allowed change. `npx playwright test --project=chromium logout-to-welcome.spec.ts` → 1 passed. No production/Statements changes. Prettier clean.)
- [x] demo (recorded `frontend/test-results/demo-logout-to-welcome.webm`; spec passed 1/1 with slowMo; config reverted, tree clean)
