...
- [S] green-domain
- [ ] adapters-discovery (expected: rest [add] — ExpiredJwtException already 422 in application.yml http-statuses, but the §5.5 detail "Activation token has expired" needs an error.handling.messages override; verified at Level-2 web slice extending AuthResourceTest.ActivateAccountTest → insert red-adapter rest / green-adapter rest; db [S] expired token throws before findById; response-shape [S] error path)
