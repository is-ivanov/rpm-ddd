...
- [S] green-usecase (no new usecase code needed)
- [~] red-domain (real-crypto Level-4 test on JwtActivationTokenGenerator.parseActivationClaim(): a token signed with a different/random secret AND a tampered-then-re-signed token both throw io.jsonwebtoken.security.SignatureException — proves real signature verification rejects tampering; regression guard mirroring 5.1/5.3, expected to pass immediately, no @ExpectedToFail. See design.)
- [ ] green-domain
- [ ] adapters-discovery (rest: NEW config mapping — SignatureException → 422 + messages override "Invalid activation token" in application.yml; verify at Level-2 web slice extending AuthResourceTest. Insert red-adapter rest / green-adapter rest. No new ports; no db/domain changes.)
