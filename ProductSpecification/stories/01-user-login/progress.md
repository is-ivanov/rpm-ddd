...
- [~] red-frontend (PER-RULE redesign — replace computePasswordStrength with evaluateComplexityRules(password): ComplexityRule[] [{key,label,met}] and its logic test; supersedes the aggregate weak/medium/strong fn + password-strength.logic.test.ts)
- [ ] green-frontend (implement evaluateComplexityRules per-rule)
