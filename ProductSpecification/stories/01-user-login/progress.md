...
- [x] red-frontend (password-strength.logic.test.ts — it.fails() RED marker, 6 Vitest cases tagged issue('189'). New pure logic fn computePasswordStrength(password): PasswordStrength ('weak'|'medium'|'strong') in password-strength.logic.ts (stub: throw 'not implemented'). NOT [S] — string→classified-level is a real transform (trivial-logic gate passed; post-impl trivial gate did not trigger). Classification (for green): score = count of the 6 activation complexity rules satisfied (≥12 chars, upper, lower, digit, special, no spaces); bands weak≤3 / medium 4-5 / strong=6; literals lowercase to bind straight to the §4.2 E2E data-strength attribute. Cases pin both E2E inputs ('weak'→weak, 'Str0ng-P@ssw0rd!'→strong) + band edges (3→weak, 4→medium, 5→medium, 6→strong) + ''→weak. PREDICT Error 'not implemented' → matched (Type/Message/Status all YES). test-review CLEAN (strict toBe per level, thresholds fully locked). refactor: renamed unused stub param→_password for lint (no impl); CLEAN otherwise. CLI lint EXIT=0; IDE clean on both files. Target: 6 expected-fail; full auth suite 20 passed | 6 expected-fail, 0 skip.)
- [x] green-frontend (computePasswordStrength implemented in password-strength.logic.ts — COMPLEXITY_RULES array of 6 predicates (length≥12, /[A-Z]/, /[a-z]/, /\d/, /[^A-Za-z0-9\s]/ special, !/\s/ no-space); score = satisfied count; bands score>=STRONG_THRESHOLD(=COMPLEXITY_RULES.length=6)→strong, >=MEDIUM_THRESHOLD(4)→medium, else weak. it.fails() markers removed (only allowed test change; assertions + issue('189') untouched). refactor: Extract Constant STRONG_THRESHOLD=COMPLEXITY_RULES.length (removed implicit 6-literal coupling so adding/removing a rule can't silently break the strong band) + MEDIUM_THRESHOLD. IDE fix: /[0-9]/→/\d/. Target 6/6; full auth suite 26/26, 0 skip, no regressions. CLI lint EXIT=0; IDE clean.)
- [S] red-frontend-api (no new .api.ts HTTP surface in §4.2 — the real-time password strength indicator is computed purely client-side from the typed password value with ZERO network requests. The feature is the pure synchronous logic fn computePasswordStrength(password): PasswordStrength in password-strength.logic.ts (built/tested in red-frontend/green-frontend): 6 complexity predicates → score → weak/medium/strong, no fetch/HTTP (grep fetch|http|axios|/api/ → no matches). The component binds the indicator's data-strength attribute to this fn on every keystroke — no endpoint, no request build, no response/error mapping, nothing for an MSW .api.ts test to stub or assert. Existence check: all auth HTTP already built/tested in prior scenarios — login.api.ts login() (§3.1/3.2), activation.api.ts validateActivationToken() (§4.1) + activateAccount()+CSRF (§5.1); §4.2 triggers none, and grep for "strength" over frontend/src finds only the logic file + its test (no .api.ts code). Zero .api.ts production files change (login.api.ts/activation.api.ts/csrf.ts untouched). Observable behavior covered by red-playwright E2E §4.2 (activation-strength.spec.ts: data-strength=weak→strong while typing) + the pure logic by password-strength.logic.test.ts. Indicator wiring built in align-design. Mirrors scenarios 1.1/2.1/5.2/6.1/2.2/3.3 red-frontend-api [S].)
- [S] green-frontend-api (no API client produced in red-frontend-api — see [S] above; the strength indicator reuses no HTTP, the data-strength binding to computePasswordStrength is built in align-design)
- [~] align-design
- [ ] green-playwright
- [ ] demo
