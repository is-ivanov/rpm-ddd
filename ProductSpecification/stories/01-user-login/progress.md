...
- [~] red-adapter rest (web slice extending AuthResourceTest.ActivateAccountTest: mock activationService.activate(token,password) to throw io.jsonwebtoken.security.SignatureException → POST /api/auth/activate with a valid-shape body (non-blank token, 12-128 char password) → assert 422 + Problem Detail detail="Invalid activation token". @Disabled until green — mapping not yet present.)
- [ ] green-adapter rest (add io.jsonwebtoken.security.SignatureException → unprocessable-content to application.yml error.handling.http-statuses + messages override "Invalid activation token"; remove @Disabled)
