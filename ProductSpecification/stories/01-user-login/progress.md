...
- [x] green-playwright (activation-strength.spec.ts §4.2 passes — test.fail() marker + stale RED comment removed; per-rule complexity indicators built in align-design. Frontend auto-started via Playwright webServer, backend mocked in-browser via page.route (no real backend; the Vite ECONNREFUSED proxy noise is the absent backend, harmless — page.route intercepts). Remove-marker-only: no production/Statements changes this unit. Ran the FULL login/activation dir to confirm the §4.1 statements migration (per-key complexity-rule-{key} locators, done in the align-design commit) stays green: 6/6 pass (§4.1 + §4.2 + §5.1/§5.2/§6.1 + Bug #188), no regressions. CLI lint EXIT=0 (oxlint/eslint/prettier/vue-tsc); IDE clean.)
- [x] demo (ran activation-strength.spec.ts §4.2 headless + slowMo 2000ms + video; 1 passed; recording frontend/test-results/demo-activation-strength-4-2.webm shows the per-rule complexity highlighting updating in real-time — partial password lights only the satisfied rules green, full password turns all 6 green; config reverted, tree clean)

### Scenario 4.3: Activation page shows error when passwords do not match
- [x] red-playwright (activation-mismatch.spec.ts §4.3 — test.fail() RED marker; new spec (33L) + ActivationMismatchStatements (47L), kept separate per the §2.2/§3.3/§4.2 one-spec-per-scenario convention. Reuses ActivationBackendStatements.givenPendingAccountForToken validate-on-load GET to render the form; the mismatch check is purely client-side (no network). When: enterMismatchedPasswords() fills password='Str0ng-P@ssw0rd!' + confirm='Different-P@ssw0rd!'. Then: assertMismatchErrorIsDisplayed() asserts the new data-testid="password-mismatch-error" visible + exact toHaveText "Passwords do not match". RED genuine: ActivationPage.vue has a confirmPassword ref but never compares it to password — no mismatch element. PREDICT toBeVisible 5s timeout on missing password-mismatch-error → matched (Type/Message/Status all YES). New testid + exact text for align-design: password-mismatch-error / "Passwords do not match" (no dedicated mockup — wording derived from spec; mockup 04-activation-form.html has no mismatch state). test-review: fixed leaked impl constants — moved the two password literals out of the Test Class into named Statements constants (enterMismatchedPasswords() now param-less, matching §4.2's typePartiallySatisfyingPassword pattern); all other checks CLEAN. refactor CLEAN — kept the 2-line navigateToActivationPageWithToken duplication vs §4.2 deliberately (extracting it would force a forbidden middleman delegator between Statements). CLI lint EXIT=0 (oxlint/eslint/prettier/vue-tsc); IDE: spec clean; statements has 4 SonarLint S2068 "hard-coded password" FPs (2 testid strings + 2 test password literals) — IDE-only, not a CI gate (per project convention; identical literals in §4.2 statements pass); // noinspection documents the testid block.)
- [~] red-frontend
- [ ] green-frontend
- [ ] red-frontend-api
- [ ] green-frontend-api
