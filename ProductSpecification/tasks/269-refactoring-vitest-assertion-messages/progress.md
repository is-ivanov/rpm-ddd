...
- [x] update green-agent "RED-Comment Reframing" (prefer message arg for per-assertion rationale) + vue-ts/tdd.md (message-arg convention; note the playwright/tdd.md precedent)
- [x] record the FE preference (memory, mirroring the backend AssertJ as() note)

### Step 2: Pilot — migrate the Scn 3.4 multi-column block
- [x] refactor (per-assertion rationale -> expect(actual, message); keep block/dataset comments)
