...
1. **Single source of filters.** New `.github/path-filters.yml` holding the named filters
   (`backend`, `frontend`, `tooling`, plus a composite used by the report jobs). Both workflows
   reference it via `dorny/paths-filter`'s `filters:` file form, so the drift that caused failure
   class 1 cannot recur.
