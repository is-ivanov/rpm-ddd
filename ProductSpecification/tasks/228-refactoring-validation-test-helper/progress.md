...
- [x] refactor (build the helper in by.iivanov.rpm.testing: concise builders for invalid argumentSet cases — NotBlank/Size/Email/composite + combined-violation cases; keep the valid-model + @ParameterizedTest shape reusable)
- [x] refactor (cleanup)

### Step 2: Refactor RegisterUserRequestTest onto the helper
- [x] refactor (replace local blankCase/sizeCase/notBlank/size/emailFormat with the shared helper; keep literal boundary limits)
- [x] refactor (cleanup)
