...
- [ ] refactor (current-user-backend.statements.ts — optional fields + DEFAULT_FIRST_NAME/DEFAULT_LAST_NAME + user={} default)

### Step 2: Drop explicit viewer-name from non-asserting call sites; keep the 3 name-asserting tests
- [ ] refactor (call sites → givenAuthenticatedUser(); keep dashboard-page / login-to-dashboard / user-menu explicit)
