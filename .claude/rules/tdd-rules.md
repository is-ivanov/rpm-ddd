...
- **Rendered-content verification → approval test at a fast level, not e2e.** Verifying the exact text/markup of rendered output (email HTML and plain-text bodies, generated templates/documents) belongs in a focused fast test that renders with **fixed, deterministic inputs** and compares the whole rendered output against a checked-in expected artifact (approval/snapshot file). Never pin full rendered content into a slow e2e test — at Level 1 assert only delivery + key invariants (recipient, from, subject, that a link is present). Determinism is mandatory: fix every variable input (clock, generated ids/tokens, random values) so the approved artifact is stable — never scrub if you can fix the input instead. See tech bindings for the per-stack mechanism.
- Acceptance tests are the "long" tests. Keep them few. Move detail to cheaper lower levels.

### Top-Tier Full-Stack Journey Assessment
Above the per-PR pyramid sits a single growing **full-stack journey** — a top-tier E2E that runs the real frontend against the real backend + real infrastructure (no mocks), on a slow cadence (nightly), covering the critical user-lifecycle path end-to-end. It is NOT one of the per-story test categories and is easy to forget — so its impact must be assessed explicitly, never implicitly.

- **Every story (and every behaviour-changing bug task) MUST record an explicit full-stack-journey verdict** — one of: **extend** an existing journey, add a **new** journey, or **no-impact** (with rationale). The verdict is a recorded artifact, not a silent omission. A missing verdict is a process gap, not "no impact".
- **A story that introduces or changes a critical user-lifecycle path MUST update the journey** (extend or new). Examples of a lifecycle-path change: a new way to drive an existing critical step through real UI (e.g. creating a user via a grid/modal instead of a raw admin API call), a new authentication/onboarding/checkout leg, a new cross-service hand-off on the happy path.
- **Prefer extending the one growing journey over forking a new one.** Add a new journey only when the new path is an independent lifecycle that does not belong on the existing journey's spine. Edge cases stay in the fast mocked suite — never in the full-stack journey.
- **No-impact is a legitimate, common verdict** — backend-only stories, pure refactors, and changes that don't touch the rendered critical path record `no-impact` with a one-line reason. The point is that the assessment happened and is auditable.
- The concrete journey location, file/suffix convention, cadence, and the extend-vs-new mechanics are tech-specific — see the browser-testing tech binding (`.claude/tech/{browser-testing}/tdd.md`). The lifecycle touchpoints (verdict at spec time, execution as a tracked step) are in `.claude/rules/workflow.md`.
...
- **green-acceptance and green-playwright are remove-marker-only phases.** The ONLY allowed change is removing the test disable/skip marker. No production code changes (backend or frontend), no Statements changes, no new files. If the test does not pass after removing the marker, STOP and report — the missing implementation belongs in an earlier phase (green-usecase, green-adapter, align-design) that was incomplete or skipped. Never "fix" backend code in a green-acceptance/green-playwright step.
  - **Exception: simple adapter plumbing.** When adapter tests are skipped `[S]` for simple delegation (no validation, no error mapping), green-acceptance may create the minimal adapter code needed for the acceptance test to pass: endpoint handler method and response DTO. No web slice test exists for these endpoints by design — the acceptance test (Level 1) covers the happy path. This exception applies only when `adapters-discovery` explicitly marked the adapter as `[S]` with "simple delegation" reason.
...
- Skip the coverage check for green-acceptance and green-playwright (black-box tests don't produce meaningful per-class coverage).
...
- **Make the reproduction DETERMINISTIC before fixing — don't fix against a hypothesis you only ran in your head.** Plain repetition often won't reproduce a failure that's really about order or shared state (it passes locally 5×, fails on CI). Once you suspect a cause, force it: turn the suspected non-deterministic variable into a controlled one and make the test fail *on demand* locally. Levers, by suspected cause: ordering coupling → **disable parallelism and pin the test execution order** to the order that should break it; concurrency race → increase parallelism / add contention; time/random → pin the clock/seed; shared-state leak → run the polluting test immediately before the victim. A controlled repro both proves the root cause and discriminates between competing diagnoses — e.g. running serially in the bad order isolates *order-dependence* from a *concurrency race* (if it still fails with parallelism off, it was never a race). Only after the failure is deterministic do you apply the fix; then re-enable the normal config and verify. The concrete knobs are tech-specific — see the tech binding's `tdd.md`.
...
1. **Test Class**: pure DSL — primarily Statements calls. No assertion library calls, no data preparation, no infrastructure, no control flow (`for`, `while`, `if`), no private methods, no inner types. **Direct SUT calls** (e.g., `sut.validateToken(token)`) are allowed — do not wrap them in a Statements method that just forwards. Statements methods that accept the SUT as a parameter to forward a single call are middlemen. Loops, conditionals, and shared setup logic belong in Statements methods — the test class should read as a flat sequence of named steps. **No implementation constants in any Statements call** — numeric IDs (task, column, board), positions, and other technical parameters belong inside Statements methods, not passed from the test class. This applies to setup (`given*`), action (`when*` — the method that invokes the SUT and captures its result/exception, matching the `// WHEN:` block; never `get*`, which reads as a getter and trips PMD `LinguisticNaming`), and assertion (`assert*`) calls equally. The test class should match the abstraction level of its test description. If the Given says "tasks in To Do and In Progress", the code should be `statements.givenTasksInTwoColumns()` — not `createTask(login, 10001L, 11001L, "TODO", ...)`. Return a structured type with the values the test needs for subsequent Statements calls.
