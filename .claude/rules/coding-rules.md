...
- **Prefer a concise one-line Javadoc; `@param`/`@return` tags are NOT required.** Checkstyle's `JavadocMethod` sets `allowMissingParamTags`/`allowMissingReturnTag` = `true`, so a single summary line (e.g. `/** Logs in as the test admin user and returns the session. */`) satisfies the check. When the parameters and return value are self-evident from the summary and signature — typical for test fixtures, Statements, and clients — do NOT add a tag-by-tag `@param`/`@return` block; it is noise. Reserve full tag blocks for genuinely non-obvious contracts (units, nullability, side effects, ranges). This complements the rule above: keep the summary line, just don't pad it.
...
- Naming: value objects = simple noun, DTOs = `{Name}Dto`, requests = `{Action}Request`, responses = `{Action}Response`, fakes = `Fake{Interface}`, persistence entities = `{Name}Entity`, read-only persistence projections (read models / `@Subselect` views) = `{Name}View`. Variables: name by source when disambiguating same-typed values in one scope.
...
- **ALL error responses use RFC 9457 — no exceptions, no native shapes.** This applies to every error the application emits, not only those reaching the centralized exception handler. Errors produced *outside* that handler — by the security/authentication layer (access-denied and unauthenticated-entry handlers) or any other framework-level error emitter that runs before or around the request handler — MUST emit the same Problem Detail shape with the same `type`-prefix convention. A library's or framework's *native* error shape (e.g. an error-handling starter's `{code, message}` body) is FORBIDDEN as an external response, even when that component ships its own default serializer. When a framework component ignores the Problem Detail setting, replace it with an adapter that emits Problem Detail.
