...
9. **Update progress** -- mark completed, advance next. **Entries are TERSE — one line: status + a short test-class/ADR ref and/or `see summaries/<slug>.md` link. Do NOT copy the phase agent's report into the checkbox: no file lists, no PREDICT tables, no lint/coverage/marker status ("collateral fixed", "it.fails()", "test.fail() removed") — that noise lives in git/commit-msg; the "why" lives in `summaries/`. See `.claude/rules/workflow.md` → "Updating Progress".**
...
10a. **Per-Phase Extended Gate** -- for stories only, when the work unit just completed was a **phase's last core scenario** (every other checkbox in that phase's section is `[x]`/`[S]`), do NOT advance to the next phase yet. **STOP** and run the Per-Phase Extended Gate (see `workflow.md` → "Per-Phase Extended Gate"): surface every still-unreviewed case in that phase's `tests/extended/NN_<Category>_Extended.md`, each with a one-line promote/defer recommendation, and **ask the user to decide per item**. Promote → append a scenario block in that phase; defer → log to `improvements.md` and mark the `### Extended` entry reviewed. This is a deliberate exception to the "never pause within a work unit" rule. If the phase has no extended file, skip silently.
10b. **Story Completion Gate** -- for stories only, when the work unit just completed was the story's **final** scenario (every other checkbox is `[x]`/`[S]`), do NOT move the row to Done yet. **STOP** and run the Story Completion Gate (see `workflow.md` → "Story Completion Gate"): surface every **still-unreviewed** `tests/extended/*_Extended.md` case (those not already decided at a phase gate) and every `Open` item in the story's `improvements.md`, each with a one-line promote/defer recommendation, and **ask the user to decide per item**. The agent never auto-promotes or auto-closes — it moves the row to Done only after the user has decided every item and any promoted scenarios are appended to `progress.md`. This is a deliberate exception to the "never pause within a work unit" rule.
