...
- Set `spring.liquibase.change-log=classpath:liquibase/changelog.xml` — real migrations from db adapter (`testRuntimeOnly project(':adapters:db')`)
...
- Liquibase migrations: `backend/adapters/db/src/main/resources/liquibase/` (provided via `testRuntimeOnly`)

## Test Configuration

`SchedulingTestConfiguration` provides:
- `@EnableAutoConfiguration` — boots PostgreSQL datasource (Testcontainers), Liquibase, Spring Scheduling
...
Liquibase runs the real migrations from the db adapter — no duplicated schema files.
